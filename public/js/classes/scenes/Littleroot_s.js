...
export default class Littleroot_s extends Phaser.Scene {
  constructor() {
    super({ key: 'Littleroot_s'});
    this.room = 'littleroot';
  ...
  preload() {
    ...
    this.load.tilemapTiledJSON('map', './js/map_littleroot.json');
  }
  ...
  update(time, delta) {
    ...
    this.info.setText('Littleroot Town\nOnline: ' + numOnline);
    this._checkKeys();
  ...
  */
  _exit() {
    ...
    this.scene.start('Oldale_s', {
      x: this.player.x,
      y: this.player.y,
      dir: 'U'
    });

    this.socket.off('init_littleroot');
